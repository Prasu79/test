...
<div class="p-4 mx-52 items-center">
        <h2 class="text-4xl dark:text-white pb-2  border-b-4 border-blue-500">Update District</h2>
        <form action="{{route('state.update',$state->id)}}"  method="POST" class="mt-5">
        @csrf
        <div>
        ...
        <input type="text" class="w-full rounded-lg mt-2 "  value="{{$state->name}}"  name="name"  placeholder="Enter Country Name">
        @error('name')
        <span  class="text-red-600 -mt-4"> *{{$message}}</span>
        @enderror
            ...
            <div class="mt-5 flex ">
                <input type="submit"  value="Updates State"  class="bg-blue-500 text-white px-3  py-2 mx-2 rounded-lg p-2">
