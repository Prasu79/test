...
<div class="my-5 text-right">
    <a href="{{route('country.create')}}" class="bg-blue-600 text-white px-3 py-2 rounded-lg">Add Country</a>
</div>
<div class="mb-8  p-3 rounded" >
    <h1 class="mb-4 dark:text-white">Search Country</h1>
    <form action="{{route('country')}}" method="GET" class="">
        @csrf
        <div class="inline dark:text-white">
            <label for="" class=""> Country Name: </label>
            <input type="search" name="search" class="dark:text-black" value="{{Request::get('name')}}" placeholder="country">
        </div>
        <button type="submit" class=" text-white px-3  py-2 hover:bg-blue-700 rounded-lg cursor-pointer bg-blue-500">Search</button>
        
        <a href="{{route('country')}} " class=" text-white px-3  py-2 rounded-lg cursor-pointer hover:bg-red-700 bg-red-500">Reset</a>
    </form>
</div>

<div class="relative overflow-x-auto">
    <h1 class="text-2xl dark:text-white mb-4">Countries</h1>  
         ...
         @foreach($countries as $country)
            ...
            <td class=" font-medium text-gray-900 whitespace-nowrap dark:text-white">
                ...
                <td class=" font-medium text-gray-900 whitespace-nowrap dark:text-white m-3 flex  ">
                    <a href="{{route('country.edit',$country->id)}}" class="font-medium text-blue-600 mx-2 p-1 dark:bg-green-800 dark:text-white  dark:rounded ">Edit</a>
                    <a onclick="showDelete('{{$country->id}}')" class="font-medium text-blue-600   dark:bg-red-800 dark:rounded p-1 dark:text-white ">Delete</a>
